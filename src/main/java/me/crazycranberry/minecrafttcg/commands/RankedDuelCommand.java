            ...
            if (args.length == 0) {
                ...
                if (playerDuelChallenges.containsKey(p)) {
                    ...
                    if (StadiumManager.stadium(p.getLocation()) != null || StadiumManager.stadium(opponent.getLocation()) != null) {
                        p.sendMessage(String.format("%sCannot accept rankedduel because one of you is already in a rankedduel.", ChatColor.GRAY));
                        opponent.sendMessage(String.format("%s%s tried to accept your rankedduel but one of you is already in a rankedduel.", ChatColor.GRAY, p.getName()));
                        return true;
                    }
                    Bukkit.getPluginManager().callEvent(new DuelAcceptedEvent((Player) sender, opponent, true));
